...
/*13位时间戳转换*/
function longtime_format($time = NULL, $format = 'Y-m-d H:i:s') {
	if (empty ( $time ))
		return '';
...
//    $mjapi = new \Addons\MijiaUser\Model\MijiaApi();
//    return $mjapi->getWxAccessToken();
...
function get_data($url, $timeout = 5){
    ...
    if (curl_errno ( $ch )) {
        $file_contents = json_encode(array('error'=>curl_error ( $ch )));
    }
...
/*
 * 兼容php5.4以上json_encode函数默认转换成unicode
 * */
...
/*订单详情提示*/
function orderStatusTips($status){
    $str = '';
    switch(intval($status)){
        case 1101:
            $str = '请确认订单后付款';
        ...
        case 1202:
            ...
            break;
        case 1203:
            $str = '建材已全部送达，请到物流运单确认收货';
            break;
        case 1204:
            $str = '订单已完成，欢迎再次购买';
            break;
        case 1301:
            $str = '工作人员将与您预约上门测量尺寸并核价';
            break;
        case 1302:
            $str = '工作人员已核价，请支付尾款';
            break;
...
/*发票类型*/
function invoiceType($invoiceType = 0){
    $str = '';
    switch(intval($invoiceType)){
        case 0:
            $str = '不开发票';
            break;
        case 1:
...
function getDiscount($discount){
    return number_format($discount/10, 1, '.', '');
}

/*转化渠道*/
function getHomeDecType($type){
    $str = '';
    switch(intval($type)){
        case 1:
            $str = '*东';
...
function JsonGet($url) {
    ...
    if (curl_errno ( $curl )) {
        ErrorLogger ( 'curl falied. Error Info: ' . curl_error ( $curl ) );
    }
    curl_close ( $curl );
    return $result;
}
...
function getJsApiTicket($token = '') {
    ...
    $tempArr = json_decode ( JsonGet($url), true );
    if (@array_key_exists ( 'ticket', $tempArr )) {
        S ( $key, $tempArr ['ticket'], 7000 );
        return $tempArr ['ticket'];
    } else {
        return 0;
    }
}
function getwxcardTicket($token = '') {
    empty ( $token ) && $token = get_token ();
    $key = 'ticket_wxcard' . $token;
    $res = S ( $key );
    if ($res !== false)
        return $res;
    $accessToken = get_access_token($token);
...
function getUser() {
    ...
    $token = I('token');
    $info = get_token_appinfo ( $token );
    $back = array('token'=>$token, 'code'=>$code, 'info'=>$info);
    if($info){
        $url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . $info['appid'] . '&secret=' . $info['secret'] . '&code=' . $code . '&grant_type=authorization_code';
        $content = get_data ( $url );
        $content = json_decode ( $content, true );
        $back = $content;

        if($back['openid'] && $back['access_token']){
            $subUrl = 'https://api.weixin.qq.com/sns/userinfo?access_token=' . $back['access_token'] . '&openid=' . $back['openid'] . '&lang=zh_CN';
            $content = get_data ( $subUrl );
            $content = json_decode ( $content, true );
