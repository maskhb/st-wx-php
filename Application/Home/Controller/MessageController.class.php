...
//		$this->assign ( 'nav', $nav );
		...
		$res ['title'] = '已发送';
		$res ['url'] = U ( 'sendall_lists2',$param );
		$res ['class'] = $act == 'sendall_lists2' ? 'current' : '';
		$nav [] = $res;
		$this->assign ( 'nav', $nav );
	}
	...
	public function add() {
		...
		if (IS_POST) {
			...
			if ($send_type == 0) {
                ...
                $_POST ['msg_id'] = $this->_send_by_project ( $project_ids );
                $m = M('message2');
                $m->create();
                $m->add();
...
2、对于认证服务号虽然使用高级群发接口的每日调用限制为100次，但是用户每月只能接收4条，无论在公众平台网站上，还是使用接口群发，用户每月只能接收4条群发消息，多于4条的群发将对该用户发送失败；' );
	...
	function _send_by_openid($openids) {
		...
		$param = JSON ( $param );
		$res = post_data ( $url, $param );
//		$res = array('msg_id'=>rand(1,10),'errcode'=>0);
		if ($res ['errcode'] != 0) {
			$this->error ( error_msg ( $res ) );
		} else {
			return $res ['msg_id'];
	...
	function _send_by_project($project_ids) {
//        dump($project_ids);exit();
	    $project_ids = wp_explode($project_ids);
        if(!$project_ids){
            $this->error ( '项目不能为空' );
        }
        $_POST['count_projects'] = count($project_ids);
        $all_projects = M('mj_project')->count();
        if($all_projects == count($project_ids)){
            $_POST['all_projects'] = 1;
        }else{
            $_POST['all_projects'] = 0;
        }

	    $openids = D('Addons://MijiaUser/MijiaUser')->getOpenidsByProjects($project_ids);
        $openids = implode(',', $openids);
        $_POST['send_openids'] = $openids;

	    return $this->_send_by_openid($openids);
	}
	// 获取素材的media_id
...
// 		$list = M ( 'custom_sendall' )->where ( $map )->order ( 'id desc' )->group ( 'diff' )->selectPage ();
	...
	function sendall_lists2(){
        ...
        foreach ( $list ['list_data'] as &$v ) {
            $openids = explode(',',$v['send_openids']);
            $v['success_count'] = count($openids);
            $v['fail_count'] =0;
            if($v['send_projects']){
                $projects = M('mj_project')->where(array('id'=>array('IN',$v['send_projects'])))->getField('name',true);
                $v['projects'] = implode('；',$projects);
            }
            if ($v ['send_type'] == 2 && $v ['all_projects'] == 1) {
                $v ['send_tip'] = '全部用户';
            }else {
                $v ['send_tip'] = '部分用户';
            }
            if ($v ['msgtype'] == 'text') {
                $v ['Content'] = '[文字] ' . $v ['content'];
            } else if ($v ['msgtype'] == 'mpnews') {
                $map ['group_id'] = $v ['appmsg_id'];
                $appMsgData = M ( 'material_news' )->where ( $map )->select ();

//                dump($appMsgData);
                foreach ( $appMsgData as $key=>&$vo ) {
                    // $art ['description'] = $vo ['intro'];
                    if (empty ( $vo ['url'] )) {
                        $art_url = U ( 'Material/news_detail', array (
                            'id' => $vo ['id']
                    ...
                    $vo['title'] = '<a style="color:inherit" href="' . $art_url . '" >' . $vo ['title'] . '</a>';
                    $v ['image_url'] = get_cover_url ( $vo ['cover_id'] );
                }
                $v['news_list'] = $appMsgData;
